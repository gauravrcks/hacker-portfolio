...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Enhanced form handling for GitHub Pages + Formspree
    const form = document.getElementById('contact-form');
    const status = document.getElementById('form-status');

    if (form) {
        form.addEventListener('submit', function (e) {
            // Don't prevent default for Formspree - let it handle the submission

            // Show loading message
            showStatus('Initializing secure transmission...', 'loading');
    ...
    function showStatus(message, type) {
        ...
        // Auto-hide loading message if there's an error
        if (type === 'loading') {
            setTimeout(() => {
    ...
    function validateForm() {
        ...
        if (!name || !email || !message) {
            showStatus('All fields are required for secure transmission', 'error');
            return false;
        }

        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!emailRegex.test(email)) {
            showStatus('Invalid email format detected', 'error');
            return false;
        }
